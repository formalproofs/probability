...
Although this document states that the values are in $\Real$, the lean file is all defined in terms of rational numbers $\mathbb{Q}$ for computability reasons. 
...
The lean file also defines several basic properties for \cref{def:probablity-value} which we do not summarize here.
...
\begin{definition}
A list $\mathbb{L}_n \colon \Nats_n \to \tau$ of type $\tau$ and length $n$ is a function defined for $\Nats_{n} := \left\{ 0, \dots , n-1 \right\}$ of type $\tau$.
%\lean{List} \leanok
\end{definition}
...
The \emph{preimage} $f^{-1}\colon \mathcal{Y} \to \mathcal{X}$ of a function $f\colon \mathcal{X} \to \mathcal{Y}$ is defined as
\begin{equation} \label{eq:preimage-set}
f^{-1}(y) := \left\{ x \in \mathcal{X} \mid  f(x) = y \right\} .
\end{equation}
%\lean{Probability.Preimage} \leanok
\end{definition}
...
%\lean{Probability.Finprob} \leanok
\end{definition}
...
\begin{definition}[Complete Probability Space] \label{def:probability-space}
  A \emph{complete probability space} consists of:
  \begin{enumerate}
  \item A sample space $\Omega := \Nats_n$, which is the set of all possible outcomes. 
  \item An \emph{implicit} event space $\mathcal{F} := 2^{\Omega}$, which we do not define explicitly in Lean. 
  \item A probability function $\mathbb{P} \in \Delta_N$ that satisfies that
  \begin{enumerate}
  \item $\mathbb{P}(\Omega) = 1$
  \item $\mathbb{P}(A_1 \cup A_2) = \mathbb{P}(A_1) + \mathbb{P}(A_2)$ for all $A_1, A_2\in \mathcal{F}$ such that $A_1 \cap A_2 = \emptyset$.
  \end{enumerate}
  \end{enumerate}
%\lean{Probability.Finprob} \leanok
\end{definition}
The notation $2^{\Omega}$ in \cref{def:probability-space} represents the power set of $\Omega$.

The set $\mathcal{F}$ is often, but not always, the power set of $\Omega$. One reason it may not be a power set is that it is necessary when $\Omega$ is infinite; see Borel sets. Another reason to define $\mathcal{F}$ that is not a power set is to denote the available information. We will see an example of this later. Unless specified otherwise, we assume that the event set for a finite sample space is its power set and the $\sigma$-algebra of $\Real$ is the Borel set.
...
\begin{example} \label{exm:probability-space}
 Suppose I want to model the behavior of coin flips. I have a coin that is equally likely to come up heads and tails. I flip the coin twice. One possible probability space would be
  \begin{enumerate}
  \item $\Omega := \left\{ \mathrm{HH}, \mathrm{HT}, \mathrm{TH}, \mathrm{TT} \right\}$.
  \item $\mathcal{F} := 2^{\Omega } = \left\{ \emptyset, \Omega, \left\{ \mathrm{HH} \right\}, \left\{ \mathrm{HH}, \mathrm{HT} \right\}, \dots  \right\}$.
  \item $P(\left\{ \mathrm{HH} \right\}) := \frac{1}{4},  P(\left\{ \mathrm{HH}, \mathrm{HT} \right\}) := \frac{1}{2}$. The probabilities of other sets are derived using the properties in \cref{def:probability-space}.
  \end{enumerate}
\end{example}
...
\begin{definition}[Random Variable]
  A \emph{random variable} is a measurable function $\tilde{x}\colon \Nats \to \mathcal{Y}$ where $\mathcal{Y}$ is a measurable set. Note that the way we define random variables, they are independent of a particular probability space. The measurability requirement is vacuous in probability spaces.
  %\lean{Probability.FinRV} \leanok
\end{definition}
...
\begin{definition}
For any complete probability space, $(\Omega, \mathcal{F}, P)$ and a random variable $\tilde{b}\colon \Omega \to \mathbb{B}$, the operator $\PP$ is defined as
\[
\PP\left[ \tilde{b} \right] := \sum_{i\in \Omega} \mathbb{P}(i) b(i),
\]
where the boolean is interpreted as $0$ for false and $1$ for true. 
%\lean{Probability.Probability.probability}
\end{definition}
...
\begin{theorem}
  ...
  \[
   \PP\left[\tilde{b} \right] := \mathbb{P}[b^{-1}(\true)]. 
 \]
 %%\lean{Probability.prob\_iprod\_eq\_def} \leanok
\end{theorem}
...
\begin{definition}[CDF]
  The \emph{cumulative distribution function} $F_{\tilde{x}}\colon \Real \to [0,1]$ of a real-valued random variable $\tilde{x}\colon \Omega \to \Real$ is defined as
  \[
   F_{\tilde{x}}(x) := \PP\left[\tilde{x} \le x\right], \qquad \forall x\in \Real. 
  \]
\end{definition}
...
The \emph{probability mass function} $p_{\tilde{x}}\colon \mathcal{E} \to [0,1]$ for a discrete random variable $\tilde{x}\colon \Omega  \to  \mathcal{E}$ with a finite $\mathcal{E}$ is defined as
 ...
 \]
%\lean{Probability.Probability.PMF} 
\end{definition}
...
\]
%%\lean{Probability.Probability.probability\_cnd}
\end{definition}
...
Suppose that $\tilde{x}\colon \Omega \to \Real $ is a random variable and that $x(\Omega) := \left\{ x(\omega) \mid  \omega \in \Omega  \right\}$ is finite. Then
\[ 
 \E\left[\tilde{x}\right] := \sum_{\omega \in \Omega }  x(\omega) \cdot  \PP(\omega).
\]
\end{definition}

The following theorem shows that our definition of expected value is equal to the standard definition of expected values. 
\begin{theorem}[Expected Value] \label{def:expectation}
Suppose that $\tilde{x}\colon \Omega \to \Real $ is a random variable and that $x(\Omega) := \left\{ x(\omega) \mid  \omega \in \Omega  \right\}$ is finite. Then
\[ 
 \E\left[\tilde{x}\right] = \sum_{x\in \tilde{x}(\Omega)}  x\cdot \PP\left[\tilde{x} = x\right].
\]
%%\lean{Probability.Probability.exp\_eq\_correct}
\end{theorem}
In general probability spaces, the expectation is defined using the Lebesgue integral:
\[
 \E\left[\tilde{x}\right] := \int_{\Omega} \tilde{x} \, dP,
\]
...
\begin{theorem}
  Assume $\tilde{x},\tilde{y}\colon \Omega \to \Real$ and $c\in \Real $. Then:
  \begin{align*}
...
\begin{theorem}[Law of the unconscious statistician] \label{thm:unconscious}
  For any discrete $\tilde{x}\colon \Omega \to \Real$ and $g\colon \Real \to \Real$:
  \[
    \E\left[g(\tilde{x})\right] = \sum_{x\in \mathcal{X}} g(x) \cdot  \PP\left[\tilde{x} = x\right].
  \]
\end{theorem}
...
\subsection{OLD CONTENT: TO BE MOVED}

Here, we define probability and expectation operators. 

\begin{definition}
A \emph{boolean} set is $\mathbb{B} = \left\{ \false, \true \right\}$.
%%\lean{Bool} \leanok
\end{definition}
...
An \emph{indicator} function $\I \colon \mathbb{B} \to \left\{ 0, 1 \right\}$ is defined for $b\in \mathbb{B}$ as
\[
\I(b) :=
\begin{cases}
...
\end{cases}
\]
%\lean{Probability.Probability.indicator} \leanok
\end{definition}

\begin{definition} \label{def:expect-cnd}
The \emph{conditional expectation} of $\tilde{x} \colon \Omega \to \Real$ conditioned on $\tilde{b} \colon \Omega \to \mathbb{B}$ is defined as
\[
...
Suppose that $\tilde{c} \colon \Omega \to \mathbb{B}$ such that $\PP\left[ \tilde{c} \right] = 0$. Then for any $\tilde{x} \colon \Omega \to \Real$:
\[
...
Suppose that $\tilde{c} \colon \Omega \to \mathbb{B}$ such that $\PP\left[ \tilde{c} \right] = 0$. Then for any $\tilde{b} \colon \Omega \to \Real$:
\[
...
Let $(\Omega,\mathcal{F},P)$ be a discrete probability space with $\Omega=\{\omega_1,\omega_2,\ldots\}$ countable, and let $p_i=P(\{\omega_i\})\geq0$ with $\sum_i p_i =1$. Let $X$ be a random variable with $X_i=X(\omega_i)$. Then
\[
E[|X|]=0 \Longleftrightarrow P(\{\omega\in\Omega:X(\omega)=0\})=1.
\]
\end{theorem}

\begin{proof}
\textit{Proof of ($\Leftarrow$):} Assume $P(X=0)=1$.
If $P(X=0)=1$, then $P(X\neq0)=0$.
...
\begin{itemize}
  \item a positive integer $S \in \Nats_{>0}$ representing the number of states, with index set $\Fin(S) = \{0,1, \dots, S-1\}$
  \item a positive integer $A \in \Nats_{>0}$ representing the number of actions, with index set $\Fin(A) = \{0,1, \dots, A-1\}$
  \item a transition function $P \colon \Fin(S) \times \Fin(A) \to \Delta(\Fin(S))$, mapping each state–action pair to finite probability distribution over next states
  \item a reward function $r \colon \Fin(S) \times \Fin(A) \times \Fin(S) \to \Real$, mapping each transition \((s,a,s')\) to a real number
\end{itemize}
